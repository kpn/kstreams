...
class TransactionalTopic(Topic):
    transaction_status: TransactionStatus = TransactionStatus.NOT_STARTED

    def __hash__(self):
        return hash(self.transaction_status.value)

    async def get(self) -> ConsumerRecord:
        while self.transaction_status in (
...
class TopicManager:
    ...
    @classmethod
    def get_transactional_topic(cls, name: str) -> TransactionalTopic:
        topic = cls.get(name)
        assert isinstance(topic, TransactionalTopic)
        return topic
